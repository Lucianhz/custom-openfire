...
public abstract class ConnectionHandler extends IoHandlerAdapter {
    ...
    public void sessionClosed(IoSession session) throws Exception {
        ...
        if ( connection != null ) {
        	...
        	JID jid = handler.getAddress();
        	String time = new Date().getTime()+"";
        	//更新群成员离线时间
        	Log.info("更新群成员离线时间"+jid.toBareJID());
        	updateAffiliatiomOfflineDateByJid(jid.toBareJID(), time);
        	updateMucmemberOfflineDateByJid(jid.toBareJID(), time);
            connection.close();
        }
    }
